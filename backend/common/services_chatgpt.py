...
from common.services_bdd import getAllTagsNames
...
#Fonction qui récupère les tags d'un lot de chansons par appel à l'API de ChatGPT
def getSongAutomaticTagsBatch(batch):
    tags = getAllTagsNames()

    tags = ', '.join(tags)

    preprompt = "You are a music tagging assistant. Your task is to determine relevant tags for a song based on what you know about it"
    preprompt += "Here are the rules you must follow:"
    preprompt += "1. If you don't know the song or artists, respond only with 'unknown' and nothing else on the row, but you should know most of the songs."
    preprompt += "2. If you feel like multiple tags fill the same role, add them all. Don’t hold back."
    preprompt += "3. Unless you don't know the song, you must provide **as many tags as possible**. Continue until you have covered all relevant dimensions of the song. Think about the song’s genre, origin, mood, tempo, themes, period, and any other attributes that could apply."
...
#Fonction qui récupère les tags pertinents pour une playlist à partir d'une description
